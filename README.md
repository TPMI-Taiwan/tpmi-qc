...
./02_report.sh {BATCH_ID_LIST} ### optional
```
...
### 2-1 Merge (04_merge)
合併所有 batches，並移除 step 1-2 和 step 1-3 挑選出(差異太大)的 SNPs。
```
./04_exe_uniq.sh
```
### 2-2 QC after merge (05_merge_qc)
將 step 2-1 合併後的檔案再 QC 一次，依序移除:
  * TPMI special SNPs
  * Duplicated SNPs
...
以 1000 Genomes Project 和 SGDP 資料為輔助，先用 PCA 分出 EAS 族群，再用 Admixture 分出 Han Chinese 族群。
### 3-1 PCA (06_pca)
...
```
### 3-3 Admixture
用 Admixture 將 Han Chinese 族群自 EAS 中區分出，包含建立 reference 及 assignment 。
...
```
將前一步驟分數超過門檻的所有樣本合起來再執行一次 Admixture。
```
cd refQ
./08_exe_refQ.sh
```
...
將其他樣本 project 到 reference。當最高分 > 0.4 且比第二高分高出 0.1，則將樣本 assign 此最高分所屬的族群。
```
./08_exe_project.sh
```
## 4. QC within Han
...
1. 挑出 TPMI 中 assign 為 Han Chinese 族群的樣本後 QC
    * 移除 SNPs missing rate > 2%
    * 移除 MAF < 0.01
2. 檢查樣本性別
...
將兩兩 batches 執行 GWAS 後移除顯著的SNPs，以減少 batch effect。
